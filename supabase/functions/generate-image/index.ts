...
serve(async (req) => {
  ...
  try {
    const { imagePrompt, projectId, segmentId, sequenceNumber, subIndex, momentType, stylePrefix, assetDescriptions, assetImageUrls } = await req.json();
    ...
    // Build text asset reference block for descriptions
    let assetBlock = '';
    ...
    const textPrompt = `ABSOLUTE REQUIREMENT: Aspect ratio 16:9 (1920x1080 widescreen).
